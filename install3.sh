...
save_port_info() {
    local port=$1
    local protocol=$2  # 新增协议类型参数: vless 或 ss
    local uuid=$3
    local private_key=$4
    local public_key=$5
    local shortid=$6
    local domain=$7
    local ss_password=$8  # 新增SS密码参数
    local ss_method=$9    # 新增SS加密方法参数
    ...
    chmod 600 "$PORT_INFO_FILE"
    log_info "保存端口 $port 配置，协议: $protocol"
}
...
EOL
    ...
    echo "$ports_config" | while read -r port_info; do
        ...
        local protocol=$(echo "$port_info" | jq -r '.protocol // "vless"') # 默认为vless协议
...
EOL
            ...
            # 处理Shadowsocks协议配置
            local password=$(echo "$port_info" | jq -r '.password')
            local method=$(echo "$port_info" | jq -r '.method')
            
            # 创建Shadowsocks的入站配置 - 修改这部分以正确配置clients数组
            cat > "$temp_config.inbound" << EOL
{
  "listen": "0.0.0.0",
  "port": ${port},
...
list_port_configurations() {
    ...
    echo -e "${cyan}序号   协议   端口    UUID/密码    域名/加密方式    代理状态    HAProxy状态${none}"
    echo "----------------------------------------------------------------"
    
    local index=1
    jq -c '.ports[]' "$PORT_INFO_FILE" | while read -r port_info; do
        local port=$(echo "$port_info" | jq -r '.port')
        local protocol=$(echo "$port_info" | jq -r '.protocol // "vless"')
        ...
        if [[ "$protocol" == "vless" ]]; then
            ...
            echo -e "${green}$index${none}    ${cyan}VLESS${none}    ${cyan}$port${none}    ${yellow}$uuid_short${none}    ${magenta}$domain${none}    ${socks5_status}    ${haproxy_status}"
        elif [[ "$protocol" == "ss" ]]; then
            local password=$(echo "$port_info" | jq -r '.password')
            # 截取密码的开头和结尾部分，中间用省略号
            local password_short="${password:0:6}...${password:(-4)}"
            local method=$(echo "$port_info" | jq -r '.method')
            ...
            echo -e "${green}$index${none}    ${cyan}SS${none}    ${cyan}$port${none}    ${yellow}$password_short${none}    ${magenta}$method${none}    ${socks5_status}    ${haproxy_status}"
        fi
...
add_port_configuration() {
    ...
    echo
    echo -e "请选择要添加的协议类型:"
    echo -e "  ${green}1.${none} VLESS+Reality"
    echo -e "  ${green}2.${none} Shadowsocks"
    echo "----------------------------------------------------------------"
    
    read -p "$(echo -e "请选择 [${green}1-2${none}]: ")" protocol_choice
    
    case $protocol_choice in
        1)
            # 添加VLESS+Reality协议
            add_vless_configuration
            ;;
        2)
            # 添加Shadowsocks协议
            add_ss_configuration
            ;;
        *)
            error
            return
...
add_vless_configuration() {
    ...
    # 保存基本端口配置
    save_port_info "$port" "vless" "$uuid" "$private_key" "$public_key" "$shortid" "$domain"
    ...
    log_info "端口 $port ($protocol) 配置成功添加"
    pause
}
...
add_ss_configuration() {
    ...
    # 端口选择
    while :; do
    read -p "$(echo -e "请输入端口 [${magenta}1-65535${none}]，建议使用大于1024的端口（默认: ${cyan}17999${none}）: ")" port
    if [[ -z "$port" ]]; then
        port=17999
    fi
    ...
    # 选择加密方式，默认aes-256-gcm
    echo -e "请选择 "$yellow"加密方式"$none" :"
    echo "  1. aes-256-gcm（默认，推荐）"
    echo "  2. aes-128-gcm"
    echo "  3. chacha20-poly1305"
    echo "  4. plain 或 none（无加密，不推荐）"
    
    read -p "$(echo -e "(默认: ${cyan}1${none}): ")" method_choice
    ...
    echo
    echo -e "$yellow 加密方式 = ${cyan}${method}${none}"
    echo "----------------------------------------------------------------"
    
    # 生成随机密码
    default_password=$(generate_random_password 16)
    echo -e "请输入 "$yellow"密码"$none" :"
    read -p "$(echo -e "(默认密码: ${cyan}${default_password}${none}): ")" password
    ...
    # 保存基本端口配置
    save_port_info "$port" "ss" "" "" "" "" "" "$password" "$method"
    ...
    # 生成SS连接信息
    generate_ss_connection_info "$port" "$password" "$method" "$ip" "$netstack"
    
    echo
    echo -e "$green Shadowsocks端口配置成功添加! $none"
    log_info "Shadowsocks端口 $port 配置成功添加"
    pause
}

# 生成Shadowsocks连接信息
generate_ss_connection_info() {
    local port=$1
    local password=$2
    local method=$3
    local ip=$4
    ...
    echo
    echo "---------- 端口 $port 的 Shadowsocks 配置信息 -------------"
    echo -e "$green ---提示..这是 Shadowsocks 服务器配置--- $none"
    echo -e "$yellow 地址 (Address) = $cyan${ip}$none"
    echo -e "$yellow 端口 (Port) = ${cyan}${port}${none}"
    echo -e "$yellow 密码 (Password) = $cyan${password}$none"
    echo -e "$yellow 加密方式 (Method) = ${cyan}${method}${none}"
    ...
    local userinfo_base64=$(echo -n "$userinfo" | base64 -w 0)
    
    ss_url="ss://${userinfo_base64}@${ip}:${port}#SS_${port}"
    
    echo
    echo "---------- Shadowsocks URL ----------"
    echo -e "${cyan}${ss_url}${none}"
    echo
    ...
    # 保存信息到文件
    local output_file="$HOME/shadowsocks_${port}.txt"
    echo "---------- 端口 $port 的 Shadowsocks 配置信息 -------------" > "$output_file"
    echo "Shadowsocks 服务器配置" >> "$output_file"
    echo "地址 (Address) = ${ip}" >> "$output_file"
    echo "端口 (Port) = ${port}" >> "$output_file"
    echo "密码 (Password) = ${password}" >> "$output_file"
    echo "加密方式 (Method) = ${method}" >> "$output_file"
    ...
    echo >> "$output_file"
    echo "---------- Shadowsocks URL ----------" >> "$output_file"
    echo "${ss_url}" >> "$output_file"
    ...
    log_info "生成Shadowsocks端口 $port 的连接信息，保存到 $output_file"
}
...
configure_socks5_for_port() {
    ...
    # 检查是否有已存在的SOCKS5出站配置
    local existing_socks5=()
    local existing_socks5_info=()
    
    # 读取端口配置文件的内容
    local port_config=$(cat "$PORT_INFO_FILE")
    
    # 使用临时文件来存储符合条件的端口信息
    local temp_file=$(mktemp)
    echo "$port_config" | jq -c '.ports[] | select(.socks5 != null and .socks5.enabled == true)' > "$temp_file"
    
    # 读取临时文件并填充数组
    while IFS= read -r port_info; do
        if [[ -n "$port_info" ]]; then
            local port_num=$(echo "$port_info" | jq -r '.port')
            local socks5_address=$(echo "$port_info" | jq -r '.socks5.address')
            local socks5_port=$(echo "$port_info" | jq -r '.socks5.port')
            
            # 排除自己的端口
            if [[ "$port_num" != "$port" ]]; then
                existing_socks5+=("$port_num")
                existing_socks5_info+=("端口:$port_num 地址:$socks5_address:$socks5_port")
            fi
        fi
    done < "$temp_file"
    ...
    # 如果有现有的SOCKS5配置，询问是否使用
    if [[ ${#existing_socks5[@]} -gt 0 ]]; then
        echo -e "${yellow}检测到已有的SOCKS5出站配置:${none}"
        local i=1
        for info in "${existing_socks5_info[@]}"; do
            echo -e "  ${cyan}$i.${none} $info"
            i=$((i+1))
        done
        echo -e "  ${cyan}$i.${none} 配置新的SOCKS5代理"
        
        local use_existing
        read -p "$(echo -e "请选择 [${green}1-$i${none}]: ")" use_existing
        
        if [[ "$use_existing" =~ ^[0-9]+$ ]] && [[ "$use_existing" -ge 1 ]] && [[ "$use_existing" -lt "$i" ]]; then
            # 使用现有的SOCKS5配置
            local selected_port=${existing_socks5[$((use_existing-1))]}
            local selected_port_info=$(jq -c ".ports[] | select(.port == $selected_port)" "$PORT_INFO_FILE")
            
            socks5_address=$(echo "$selected_port_info" | jq -r '.socks5.address')
            socks5_port=$(echo "$selected_port_info" | jq -r '.socks5.port')
            auth_needed=$(echo "$selected_port_info" | jq -r '.socks5.auth_needed')
            socks5_user=$(echo "$selected_port_info" | jq -r '.socks5.username')
            socks5_pass=$(echo "$selected_port_info" | jq -r '.socks5.password')
            udp_over_tcp=$(echo "$selected_port_info" | jq -r '.socks5.udp_over_tcp')
            
            # 根据auth_needed决定是否显示认证信息
            if [[ "$auth_needed" == "true" ]]; then
                echo -e "${green}使用现有SOCKS5配置: $socks5_address:$socks5_port (使用认证: ${socks5_user})${none}"
            else
                echo -e "${green}使用现有SOCKS5配置: $socks5_address:$socks5_port (不使用认证)${none}"
            fi
            ...
            # HAProxy配置
            local haproxy_port=$((port + 10))
            # 检查端口是否被占用
            while lsof -i:"$haproxy_port" >/dev/null 2>&1; do
                echo -e "${yellow}端口 $haproxy_port 已被占用，自动选择下一个可用端口${none}"
            ...
            log_info "为端口 $port 自动配置HAProxy，端口: $haproxy_port"
...
modify_port_configuration() {
    ...
    while :; do
        ...
        local protocol=$(echo "$selected_port_info" | jq -r '.protocol // "vless"')
        
        echo
        echo -e "$yellow 正在修改端口 ${cyan}$port${none} 的配置 (协议: ${cyan}$protocol${none}) $none"
        break
    done
    ...
    # 尝试读取这个端口最初使用的网络栈类型
    local port_connection_file=""
    if [[ "$protocol" == "vless" ]]; then
        port_connection_file="$HOME/vless_reality_${port}.txt"
    elif [[ "$protocol" == "ss" ]]; then
        port_connection_file="$HOME/shadowsocks_${port}.txt"
    fi
    ...
    if [[ "$protocol" == "vless" ]]; then
        ...
        # Shadowsocks 协议修改菜单
        echo "----------------------------------------------------------------"
        echo -e "  ${green}1.${none} 修改密码"
        echo -e "  ${green}2.${none} 修改加密方式"
        echo -e "  ${green}3.${none} 修改SOCKS5代理设置"
        echo -e "  ${green}4.${none} 管理HAProxy设置"
        ...
        case $modify_choice in
            1)
                # 修改密码
                modify_ss_password "$port"
                ;;
                
            2)
                # 修改加密方式
                modify_ss_method "$port"
                ;;
    ...
    if [[ "$original_config_md5" != "$new_config_md5" ]]; then
        ...
        if [[ "$protocol" == "vless" ]]; then
            ...
            # 使用之前保存的网络环境信息
            generate_ss_connection_info "$port" "$password" "$method" "$current_ip" "$current_netstack"
        fi
    else
...
modify_ss_password() {
    local port=$1
    local port_info=$(get_port_info "$port")
    local old_password=$(echo "$port_info" | jq -r '.password')
    
    echo
    echo -e "$yellow 修改Shadowsocks密码 $none"
    echo "----------------------------------------------------------------"
    echo -e "当前密码: $cyan$old_password$none"
    
    # 生成新的默认密码
    local default_password=$(generate_random_password 16)
    
    echo -e "请输入新的密码"
    read -p "$(echo -e "(留空使用随机密码: ${cyan}${default_password}${none}): ")" new_password
    [ -z "$new_password" ] && new_password=$default_password
    
    echo
    echo -e "$yellow 新密码 = ${cyan}$new_password${none}"
    
    # 保存修改
    local method=$(echo "$port_info" | jq -r '.method')
    save_port_info "$port" "ss" "" "" "" "" "" "$new_password" "$method"
    ...
    success "Shadowsocks密码修改成功!"
    log_info "修改端口 $port 的Shadowsocks密码: $old_password -> $new_password"
}
...
modify_ss_method() {
    local port=$1
    local port_info=$(get_port_info "$port")
    local old_method=$(echo "$port_info" | jq -r '.method')
    
    echo
    echo -e "$yellow 修改Shadowsocks加密方式 $none"
    echo "----------------------------------------------------------------"
    echo -e "当前加密方式: $cyan$old_method$none"
    ...
    read -p "$(echo -e "请选择 [${green}1-4${none}]: ")" method_choice
    
    local new_method
    case $method_choice in
        1)
            new_method="aes-256-gcm"
            ;;
        2)
            new_method="aes-128-gcm"
            ;;
        3)
            new_method="chacha20-poly1305"
            ;;
        4)
            new_method="none"
            ;;
    ...
    echo
    echo -e "$yellow 新加密方式 = ${cyan}$new_method${none}"
    
    # 保存修改
    local password=$(echo "$port_info" | jq -r '.password')
    save_port_info "$port" "ss" "" "" "" "" "" "$password" "$new_method"
    ...
    success "Shadowsocks加密方式修改成功!"
    log_info "修改端口 $port 的Shadowsocks加密方式: $old_method -> $new_method"
}
...
modify_port_uuid() {
    ...
    while :; do
        ...
        # 保存修改
        local domain=$(echo "$port_info" | jq -r '.domain')
        save_port_info "$port" "vless" "$new_uuid" "$new_private_key" "$new_public_key" "$new_shortid" "$domain"
...
modify_port_domain() {
    ...
    save_port_info "$port" "vless" "$uuid" "$private_key" "$public_key" "$shortid" "$new_domain"
...
modify_port_shortid() {
    ...
    while :; do
        ...
        save_port_info "$port" "vless" "$uuid" "$private_key" "$public_key" "$new_shortid" "$domain"
...
delete_port_configuration() {
    ...
    while :; do
        ...
        echo
        echo -e "$yellow 确认要删除端口 ${cyan}$port${none} (协议: ${cyan}$protocol${none}) 的配置吗? $none"
        ...
        if [[ "$confirm_delete" == "y" ]]; then
            ...
            if [[ "$protocol" == "vless" ]]; then
                ...
                local connection_file="$HOME/shadowsocks_${port}.txt"
            fi
...
uninstall_xray() {
    ...
    # 删除连接信息文件(vless_reality开头和shadowsocks开头的txt文件)
    rm -f "$HOME/vless_reality_"*.txt "$HOME/shadowsocks_"*.txt 2>/dev/null
...
show_all_connections() {
    ...
    # 显示所有配置的端口列表
    echo -e "${cyan}序号   协议   端口    UUID/密码    域名/加密方式${none}"
    echo "----------------------------------------------------------------"
    ...
    jq -c '.ports[]' "$PORT_INFO_FILE" | while read -r port_info; do
        ...
        if [[ "$protocol" == "vless" ]]; then
            ...
            echo -e "${green}$index${none}    ${cyan}VLESS${none}    ${cyan}$port${none}    ${yellow}$uuid_short${none}    ${magenta}$domain${none}"
        elif [[ "$protocol" == "ss" ]]; then
            local password=$(echo "$port_info" | jq -r '.password')
            # 截取密码的开头和结尾部分，中间用省略号
            local password_short="${password:0:6}...${password:(-4)}"
            ...
            echo -e "${green}$index${none}    ${cyan}SS${none}    ${cyan}$port${none}    ${yellow}$password_short${none}    ${magenta}$method${none}"
        fi
    ...
    for ((i=0; i<$port_count; i++)); do
        local port_info=$(jq -c ".ports[$i]" "$PORT_INFO_FILE")
        local port=$(echo "$port_info" | jq -r '.port')
        local protocol=$(echo "$port_info" | jq -r '.protocol // "vless"')
        echo -e "  ${green}$display_index.${none} 显示端口 ${cyan}$port${none} (协议: ${cyan}$protocol${none}) 的连接信息"
    ...
    while true; do
        ...
        if [[ "$view_choice" == "0" ]]; then
            ...
            # 根据协议类型显示不同的连接信息
            echo
            echo -e "$yellow 显示端口 ${cyan}$port${none} (协议: ${cyan}$protocol${none}) 的连接信息 $none"
            echo "----------------------------------------------------------------"
            ...
            if [[ "$protocol" == "vless" ]]; then
                ...
                local password=$(echo "$selected_port_info" | jq -r '.password')
                local method=$(echo "$selected_port_info" | jq -r '.method')
...
generate_haproxy_config_direct() {
    ...
    # 清空端口信息文件中的ports数组
    jq '.ports = []' "$PORT_INFO_FILE" > "${PORT_INFO_FILE}.tmp"
    mv "${PORT_INFO_FILE}.tmp" "$PORT_INFO_FILE"
    ...
    if $haproxy_exists; then
        ...
        while IFS= read -r line; do
            ...
            # 从frontend中提取HAProxy端口
            if [[ "$state" == "frontend" && "$line" =~ bind\ \*:([0-9]+) ]]; then
            ...
            # 从backend中提取SOCKS5地址和端口
            if [[ "$state" == "backend" && "$line" =~ server\ socks1\ ([^:]+):([0-9]+) ]]; then
                ...
                if [[ -n "$vless_port" && -n "$haproxy_port" && -n "$socks5_address" && -n "$socks5_port" ]]; then
                    ...
                    echo -e "${green}读取到HAProxy配置: 端口=$vless_port, HAProxy端口=$haproxy_port, SOCKS5=$socks5_address:$socks5_port${none}"
                fi
    ...
    while read -r inbound; do
        ...
        if [[ "$protocol" == "vless" ]]; then
            ...
            if [[ "$security" == "reality" ]]; then
                ...
                # 遍历路由规则寻找SOCKS5出站
                local rules=$(jq -c '.routing.rules[]' "$CONFIG_FILE")
                for rule in $rules; do
                    if echo "$rule" | jq -e --arg tag "$tag" '.inboundTag[] | select(. == $tag)' >/dev/null; then
                ...
                # 初始化端口信息
                local port_info="{\"port\": $port, \"protocol\": \"vless\", \"uuid\": \"$uuid\", \"private_key\": \"$private_key\", \"public_key\": \"$public_key\", \"shortid\": \"$shortid\", \"domain\": \"$domain\""
                ...
                if [[ -n "$socks5_tag" ]]; then
                    ...
                    if [[ -n "$socks5_outbound" ]]; then
                        local socks5_address=$(echo "$socks5_outbound" | jq -r '.settings.servers[0].address')
                        local socks5_port=$(echo "$socks5_outbound" | jq -r '.settings.servers[0].port')
                        
                        # 验证地址和端口
                        if [[ "$socks5_address" == "null" || -z "$socks5_address" ]]; then
                            echo -e "${yellow}端口 $port 的SOCKS5地址为空，跳过SOCKS5配置${none}"
                            ...
                            if echo "$socks5_outbound" | jq -e '.settings.servers[0].users' > /dev/null; then
                                auth_needed="true"
                                ...
                                # 验证认证信息
                                if [[ -z "$socks5_user" || -z "$socks5_pass" || "$socks5_user" == "null" || "$socks5_pass" == "null" ]]; then
                                    echo -e "${yellow}端口 $port 的SOCKS5认证信息不完整，请输入:${none}"
                                    echo -e "用户名:"
                                    read -p "> " socks5_user
                                    echo -e "密码:"
                                    read -p "> " socks5_pass
                                    ...
                                    if [[ -z "$socks5_user" || -z "$socks5_pass" ]]; then
                                        ...
                                        echo -e "${yellow}使用默认认证信息: $socks5_user / $socks5_pass${none}"
                                    fi
                                fi
                            fi
                            ...
                            if echo "$socks5_outbound" | jq -e '.streamSettings.sockopt.udpFragmentSize' > /dev/null; then
                                udp_over_tcp="true"
                            fi
                            
                            # 检查是否本地地址
                            local is_local_address=false
                            if [[ "$socks5_address" == "127.0.0.1" || "$socks5_address" == "localhost" ]]; then
                                is_local_address=true
                                echo -e "${yellow}端口 $port 的SOCKS5地址为本地地址 ($socks5_address)${none}"
                            fi
                            
                            # 查找现有HAProxy配置
                            local existing_haproxy=""
                            local current_haproxy_port=""
                            
                            for config in "${haproxy_configs[@]}"; do
                                if echo "$config" | jq -e --arg port "$port" '.vless_port == ($port | tonumber)' > /dev/null; then
                                    existing_haproxy="$config"
                                    current_haproxy_port=$(echo "$config" | jq -r '.haproxy_port')
                                    break
                                fi
                            done
                            
                            # 根据地址类型和现有配置处理HAProxy
                            if $is_local_address; then
                                if [[ -n "$existing_haproxy" ]]; then
                                    local haproxy_socks5_address=$(echo "$existing_haproxy" | jq -r '.socks5_address')
                                    
                                    if [[ "$haproxy_socks5_address" == "127.0.0.1" || "$haproxy_socks5_address" == "localhost" ]]; then
                                        echo -e "${red}端口 $port 的HAProxy配置也使用了本地地址，不会配置HAProxy${none}"
                                        port_info="$port_info, \"socks5\": {\"enabled\": true, \"address\": \"$socks5_address\", \"port\": $socks5_port, \"auth_needed\": $auth_needed, \"username\": \"$socks5_user\", \"password\": \"$socks5_pass\", \"udp_over_tcp\": $udp_over_tcp}, \"haproxy\": null"
                                    else
                                        echo -e "${green}端口 $port 的HAProxy配置使用非本地地址，保留现有配置${none}"
                                        port_info="$port_info, \"socks5\": {\"enabled\": true, \"address\": \"$haproxy_socks5_address\", \"port\": $(echo "$existing_haproxy" | jq -r '.socks5_port'), \"auth_needed\": $auth_needed, \"username\": \"$socks5_user\", \"password\": \"$socks5_pass\", \"udp_over_tcp\": $udp_over_tcp}, \"haproxy\": {\"enabled\": true, \"port\": $current_haproxy_port, \"threads\": 12, \"maxconn\": 400}"
                                ...
                                # 非本地地址
                                if [[ -n "$existing_haproxy" ]]; then
                                    echo -e "${green}使用现有HAProxy配置: 端口=$current_haproxy_port${none}"
                                    port_info="$port_info, \"socks5\": {\"enabled\": true, \"address\": \"$socks5_address\", \"port\": $socks5_port, \"auth_needed\": $auth_needed, \"username\": \"$socks5_user\", \"password\": \"$socks5_pass\", \"udp_over_tcp\": $udp_over_tcp}, \"haproxy\": {\"enabled\": true, \"port\": $current_haproxy_port, \"threads\": 12, \"maxconn\": 400}"
                                else
                                    # 创建新的HAProxy配置
                                    local new_haproxy_port=$((port + 1))
                                    while lsof -i:"$new_haproxy_port" >/dev/null 2>&1; do
                                        new_haproxy_port=$((new_haproxy_port + 1))
                                    done
                                    echo -e "${green}为端口 $port 创建新的HAProxy配置，端口: $new_haproxy_port${none}"
                                    port_info="$port_info, \"socks5\": {\"enabled\": true, \"address\": \"$socks5_address\", \"port\": $socks5_port, \"auth_needed\": $auth_needed, \"username\": \"$socks5_user\", \"password\": \"$socks5_pass\", \"udp_over_tcp\": $udp_over_tcp}, \"haproxy\": {\"enabled\": true, \"port\": $new_haproxy_port, \"threads\": 12, \"maxconn\": 400}"
                                fi
                                echo "true" > "$NEEDS_HAPROXY_FILE"
                            }
                            fi
                        ...
                        echo -e "${yellow}未找到端口 $port 的SOCKS5出站配置${none}"
                        port_info="$port_info, \"socks5\": null, \"haproxy\": null"
                    }
                    fi
                else {
                ...
                # 保存到端口信息文件
                jq ".ports += [$(echo $port_info)]" "$PORT_INFO_FILE" > "${PORT_INFO_FILE}.tmp"
                mv "${PORT_INFO_FILE}.tmp" "$PORT_INFO_FILE"
                chmod 600 "$PORT_INFO_FILE"
                
                echo -e "${green}已添加端口 $port 的VLESS配置信息${none}"
            fi
        # 处理Shadowsocks协议
            ...
            # 兼容两种配置格式
            if echo "$inbound" | jq -e '.settings.password' >/dev/null && echo "$inbound" | jq -r '.settings.password' != "null"; then
                # 旧格式配置
                password=$(echo "$inbound" | jq -r '.settings.password')
                method=$(echo "$inbound" | jq -r '.settings.method')
            elif echo "$inbound" | jq -e '.settings.clients[0]' >/dev/null; then
                # 新格式配置
                password=$(echo "$inbound" | jq -r '.settings.clients[0].password')
                method=$(echo "$inbound" | jq -r '.settings.clients[0].method')
            fi
            
            # 验证密码和加密方式
            if [[ "$password" == "null" || -z "$password" ]]; then
                password=$(generate_random_password 16)
                echo -e "${yellow}端口 $port 的Shadowsocks密码为空，已自动生成: $password${none}"
            fi
            
            if [[ "$method" == "null" || -z "$method" ]]; then
                method="aes-256-gcm"
                echo -e "${yellow}端口 $port 的Shadowsocks加密方式为空，使用默认值: $method${none}"
            fi
            ...
            # 初始化端口信息
            local port_info="{\"port\": $port, \"protocol\": \"ss\", \"password\": \"$password\", \"method\": \"$method\""
            ...
            if [[ -n "$socks5_tag" ]]; then
                ...
                if [[ -n "$socks5_outbound" ]]; then
                    ...
                    # 验证地址和端口
                    if [[ "$socks5_address" == "null" || -z "$socks5_address" || "$socks5_port" == "null" || -z "$socks5_port" ]]; then
                        echo -e "${yellow}端口 $port 的SOCKS5地址或端口为空，跳过SOCKS5配置${none}"
                        port_info="$port_info, \"socks5\": null, \"haproxy\": null"
                    else
                        # 认证设置
                        local auth_needed="false"
            ...
            echo -e "${green}已添加端口 $port 的Shadowsocks配置信息${none}"
        fi
    done < "$TEMP_FILE"
...
show_help() {
    ...
    echo -e "  ${green}2.${none} 添加新端口配置: 添加新的 VLESS Reality 或 Shadowsocks 端口，并可选配置 SOCKS5 代理。"
    echo -e "  ${green}3.${none} 查看所有端口配置: 显示当前配置的所有端口信息。"
    echo -e "  ${green}4.${none} 修改端口配置: 修改现有端口的配置（VLESS或SS）。"
    ...
    echo -e "  ${green}8.${none} 备份与恢复: 备份当前配置或从备份恢复配置。"
    echo -e "  ${green}9.${none} 查看日志: 显示 Xray 运行日志。"
    echo -e "  ${green}10.${none} 卸载 Xray: 完全卸载 Xray 和相关配置。"
    echo -e "  ${green}11.${none} 根据当前Xray配置自动同步端口信息: 从现有配置中同步端口信息，适合迁移。"
    echo -e "  ${green}0.${none} 退出: 退出脚本。"
    echo "----------------------------------------------------------------"
    echo -e "当前版本: ${cyan}$VERSION${none}"
    echo -e "支持协议: ${cyan}VLESS+Reality, Shadowsocks${none}"
    echo "----------------------------------------------------------------"
...
show_menu() {
    ...
    while $keep_running; do
        ...
        echo -e "  ${green}12.${none} 根据当前Xray配置自动同步端口信息"
