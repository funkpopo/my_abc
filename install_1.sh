...
EOL
        ...
        if [[ "$socks5_config" != "null" && "$(echo "$socks5_config" | jq -r '.enabled')" == "true" ]]; then
            # 由于默认启用HAProxy，我们使用HAProxy的端口
            local haproxy_config=$(echo "$port_info" | jq -r '.haproxy')
            local socks5_address="127.0.0.1"  # 使用本地地址作为HAProxy监听地址
            local socks5_port
...
configure_socks5_for_port() {
    ...
    while lsof -i:"$haproxy_port" >/dev/null 2>&1; do
        echo -e "${yellow}端口 $haproxy_port 已被占用，请指定其他端口${none}"
        read -p "$(echo -e "请输入 HAProxy 端口: ")" haproxy_port
        if [ -z "$haproxy_port" ] || ! [[ "$haproxy_port" =~ ^[0-9]+$ ]] || [[ "$haproxy_port" -lt 1 ]] || [[ "$haproxy_port" -gt 65535 ]]; then
            error
            haproxy_port=$((port + 1))
            continue
        fi
...
modify_port_socks5() {
    ...
    if [[ "$socks5_config" != "null" && "$(echo "$socks5_config" | jq -r '.enabled // false')" == "true" ]]; then
        ...
        if [[ "$disable_socks5" == "y" ]]; then
            ...
            local haproxy_config=$(echo "$port_info" | jq -r '.haproxy')
            if [[ "$haproxy_config" != "null" && "$(echo "$haproxy_config" | jq -r '.enabled // false')" == "true" ]]; then
                echo -e "${yellow}SOCKS5代理配置已更新，正在更新HAProxy配置...${none}"
                update_haproxy_config
            fi
            ...
            # 添加HAProxy配置修改部分
            echo
            echo -e "$yellow 修改HAProxy设置 $none"
            echo "----------------------------------------------------------------"
            ...
            if [[ "$haproxy_config" != "null" && "$(echo "$haproxy_config" | jq -r '.enabled // false')" == "true" ]]; then
                ...
                echo
                echo -e "是否修改HAProxy配置?"
                read -p "$(echo -e "(y/n, 默认: ${cyan}n${none}): ")" modify_haproxy
                
                if [[ "$modify_haproxy" == "y" ]]; then
...
generate_haproxy_config_direct() {
    ...
    # 提取所有inbound的标签和端口
    jq -r '.inbounds[] | "\(.tag)|\(.port)"' "$CONFIG_FILE" > "$temp_file"
...
EOL
    ...
    # 检查是否有任何SOCKS5出站
    local socks5_tags=$(jq -r '.outbounds[] | select(.protocol == "socks") | .tag' "$CONFIG_FILE")
    
    if [[ -z "$socks5_tags" ]]; then
        echo -e "${yellow}在Xray配置中没有找到任何SOCKS5出站${none}"
        rm -f "$temp_file"
        return 0
    fi
            ...
            if [[ "$security" == "reality" ]]; then
                ...
                if [[ -n "$socks5_tag" ]]; then
                    ...
                    if [[ "$socks5_address" == "1" || "$socks5_address" == "localhost" || "$socks5_port" -eq 1 ]]; then
                        ...
                        echo -e "请输入正确的SOCKS5服务器端口:"
                        read -p "$(echo -e "> ")" new_socks5_port
                        
                        if [[ -n "$new_socks5_address" && -n "$new_socks5_port" ]]; then
...
backend socks_servers_$port
    server socks1 $socks5_address:$socks5_port maxconn 200 check inter 5000 rise 2 fall 3
...
EOL
    ...
    # 重启HAProxy服务
    echo -e "${yellow}正在重启HAProxy服务...${none}"
    if systemctl restart haproxy; then
        echo -e "${green}HAProxy服务已重启${none}"
    else
