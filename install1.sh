...
EOL
            ...
            # 创建DNS出站配置，使用SOCKS5代理的地址
            cat > "$temp_config.dns" << EOL
{
  "protocol": "dns",
  "settings": {
    "address": "${socks5_address}",
...
EOL
            ...
            # 添加DNS出站到主配置
            jq ".outbounds += [$(cat "$temp_config.dns")]" "$temp_config" > "$temp_config.new"
            mv "$temp_config.new" "$temp_config"
...
EOL
    ...
    # 清理临时文件
    rm -f "$temp_config" "$temp_config.inbound" "$temp_config.socks5" "$temp_config.dns" "$temp_config.rule" "$temp_config.dns_rule" "$temp_config.new" 2>/dev/null
